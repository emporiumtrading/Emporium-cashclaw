...
export const DEFAULT_PREDICTION_CONFIG: PredictionConfig = {
  maxPositionPct: 5,           // Max 5% of balance per trade
  maxTotalExposurePct: 25,     // Max 25% of balance exposed total
  minConfidence: 0.65,         // Only trade when 65%+ confident
  maxPositions: 5,             // Max 5 concurrent positions
  stopLossPct: 20,             // Stop loss at 20% down
  dailyLossLimitUsd: 50,      // Stop trading if down $50 in a day
};
...
export function recordTrade(position: Position): void {
  const db = getDb();
  db.prepare(`
    INSERT OR REPLACE INTO predictions (id, market, platform, outcome, entry_price, quantity, cost_basis, current_value, confidence, thesis, status, pnl, opened_at, closed_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(position.id, position.market, position.platform, position.outcome, position.entryPrice, position.quantity, position.costBasis, position.currentValue, position.confidence, position.thesis, position.status, position.pnl ?? 0, position.openedAt, position.closedAt ?? null);
...
export function closeTrade(id: string, pnl: number): void {
  const db = getDb();
  const status = pnl >= 0 ? "closed" : "stopped";
  db.prepare("UPDATE predictions SET status = ?, pnl = ?, current_value = cost_basis + ?, closed_at = ? WHERE id = ?")
    .run(status, pnl, pnl, Date.now(), id);
